...
type Tab = 'sports' | 'courses' | 'ManageCategoriesPage' | 'CourseCategoryMappingPage' | 'add course schedule' | 'add course schedule rule' | 'Manage Daily Availability Page' | 'add package schedule' |  'add package schedule' |  'add Package schedule rule' ;
...
const ManageSportsAndCoursesPage: React.FC = () => {
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-gray-800 mb-4">Sports & Courses</h2>
      <p className="text-gray-600 mb-8">
        Manage all sports categories and their corresponding course offerings from this central hub.
      </p>
      ...
      <div className="mt-8">
         {activeTab === 'courses' && <CoursesManagement />}
          ...
          {activeTab === 'add course schedule' && <CourseSchedulesManagement />}
           {activeTab === 'add course schedule rule' && <CourseScheduleManagement />} 
           {activeTab === 'add package schedule' && <PackageSchedulesManagement />} 
            {activeTab === 'add Package schedule rule' && <PackageScheduleRuleManagement />}
            {activeTab === 'Manage Daily Availability Page' && <ManageDailyAvailabilityPage />}
            {activeTab === 'ManageCategoriesPage' && <ManageCategoriesPage />}
             {activeTab === 'CourseCategoryMappingPage' && <CourseCategoryMappingPage />}
      </div>
    </div>
  );
};
