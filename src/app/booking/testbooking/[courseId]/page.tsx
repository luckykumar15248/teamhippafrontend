...
const BookingPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            // Only restore from localStorage if there's no logged-in user
    ...
    }, [bookingDataKey, router]);

    useEffect(() => {
        // Don't save personal info to localStorage if user is logged in
        if (!currentUser) {
    ...
    // --- Component Logic ---
    const addParticipant = () => setParticipants([...participants, { id: Date.now(), firstName: '', lastName: '', dateOfBirth: '', gender: 'Prefer not to say', skillLevel: 'Beginner', medicalNotes: '', emergencyContactName: '', emergencyContactPhone: '' }]);
    const removeParticipant = (id: number) => {
        if (participants.length > 1) setParticipants(participants.filter(p => p.id !== id));
    ...
    const handleDateSelect = (dateString: string) => {
        setSelectedDates(prev => {
            ...
            if (newSet.has(dateString)) newSet.delete(dateString); else newSet.add(dateString);
            return newSet;
        });
    };
...
const priceDetails = useMemo(() => {
        ...
        if (discount) {
            if (discount.type === 'PERCENTAGE') {
                discountAmount = totalSubtotal * (discount.amount / 100);
            } else {
                discountAmount = discount.amount;
            }
        }
...
const isTodayOrFuture = (date: Date) => {
    ...
    console.log("Booking payload:");
        ...
        try {
            const bookingPayload = {
                userId: currentUser?.id,// Placeholder for user ID, replace with actual user context if available
            ...
            // CORRECTED: This now calls the initiation endpoint
            ...
            if (response.data.success) {
                ...
                localStorage.removeItem(bookingDataKey); // Clear saved progress
                router.push(`/checkout/${bookingId}`); // Redirect to the checkout page
            } else {
  ...
  <select
    value={p.dailyHours || ''}
    onChange={e => handleParticipantChange(index, 'dailyHours', (e.target.value))}
  ...
  >
    ...
    const isFutureOrToday = isTodayOrFuture(day);
    ...
    return (
        <div 
            key={dayString} 
            onClick={() => isAvailable && isFutureOrToday && handleDateSelect(dayString)} 
