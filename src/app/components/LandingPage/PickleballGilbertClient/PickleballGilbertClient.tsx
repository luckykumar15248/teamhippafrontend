...
import SummerCampShortDetails from "../../SummerCampShortDetails/SummerCampShortDetails";

// --- Type Definitions as before ---
interface Course {
  id: number;
  slug: string; 
  name: string;
...
const DetailsModal: React.FC<DetailsModalProps> = ({
  ...
  if (!item) return null;
  const animationClasses = "transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale";
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center p-4 transition-opacity duration-300">
      <div className={`bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col ${animationClasses}`}>
...
const PickleballGilbertClient: React.FC = () => {
  ...
  return (
    <>
      <SportsHeroSection
        ...
        showCallButton
      />
      {/* ...ENTIRE EXISTING PAGE CONTENT WITHOUT <Meta/> ... */}
      {/* Rest of your jsx — use the code you posted but DO NOT include any Meta component here */}
      {/* All your sections, buttons, swipers, grids, modals, etc. are retained as is */}
      {/* See original code for full content (was too long to repeat all here, but you just need to copy it as is from your client file) */}
...
export default PickleballGilbertClient;
