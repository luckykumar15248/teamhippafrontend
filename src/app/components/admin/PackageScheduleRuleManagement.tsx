// File: app/(dashboard)/admin/package-rules/page.tsx
// A complete, standalone page for managing the booking rules for package schedules.
...
const PackageBookingRulesPage = () => {
    ...
    const fetchRulesForSchedule = useCallback(async (scheduleId: number) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.get(`${apiUrl}api/admin/package-booking-rules/schedule/${scheduleId}`, { headers });
    ...
    const handleDeleteRule = async () => {
        ...
        try {
            await axios.delete(`${apiUrl}api/admin/package-booking-rules/${ruleToDelete.rule_id}`, { headers });
            toast.success("Rule deleted successfully.");
            setRules(prev => prev.filter(rule => rule.rule_id !== ruleToDelete.rule_id));
        } catch (error: any) {
        ...
        <div className="bg-gray-100 min-h-screen font-sans">
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2 sm:mb-0">Package Booking Rules</h1>
                ...
                <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="packageSelector" className="block text-sm font-medium text-gray-700 mb-1">1. Select a Package</label>
                        <select
                            id="packageSelector"
                     ...
                     <div>
                        <label htmlFor="scheduleSelector" className="block text-sm font-medium text-gray-700 mb-1">2. Select a Schedule</label>
                        <select
                            id="scheduleSelector"
                        ...
                        >
                            ...
                            {schedules.map(s => <option key={s.scheduleId} value={s.scheduleId}>{s.name}</option>)}
                        </select>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-12 bg-white rounded-lg shadow-md">
                        <p className="text-gray-500">Please select a package to see its available schedules.</p>
                    </div>
                ) : !selectedScheduleId && schedules.length > 0 ? (
                    <div className="text-center py-12 bg-white rounded-lg shadow-md">
                        <p className="text-gray-500">Please select a schedule to view or add booking rules.</p>
                    </div>
                ) : (
...
const RuleModal: React.FC<RuleModalProps> = ({ isOpen, onClose, onSave, rule, schedules, selectedScheduleId }) => {
    ...
    const [selectedSchedule, setSelectedSchedule] = useState<PackageSchedule | null>(null);
    ...
    useEffect(() => {
        const schedule = schedules.find(s => s.scheduleId === Number(formData.schedule_id));
                ...
                <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto">
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div>
                                <label htmlFor="schedule_id" className="block text-sm font-medium text-gray-700">Package Schedule</label>
                        ...
                        <div className="flex justify-between items-center mt-8 mb-4 border-b pb-2">
                            ...
                            {selectedSchedule && <p className="text-sm text-gray-500">Schedule valid: {selectedSchedule.startDate} to {selectedSchedule.endDate}</p>}
                        </div>
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                            {weekDays.map(day => (
                                <label key={day} htmlFor={`day_${day}`} className="flex items-center space-x-2 text-sm cursor-pointer">
