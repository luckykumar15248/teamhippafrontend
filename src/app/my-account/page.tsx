...
const PackageIcon = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7m0 0v10l8 4"
    />
  </svg>
);
...
// --- Child Components ---
const BookingCard: React.FC<{ booking: UserBookingDetails }> = ({
  booking,
}) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <button
        ...
        className="w-full p-4 text-left focus:outline-none hover:bg-gray-50 transition-colors"
      >
        <div className="flex items-center justify-between">
          ...
          <div className="text-right flex-shrink-0 ml-4">
            <div
              className={`mb-1 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getTypeClass(
                booking.bookingType
              )}`}
            >
              {booking.bookingType}
...
const UserDashboardPage: React.FC = () => {
  ...
  const [selectedPackage, setSelectedPackage] =
    useState<PurchasedPackage | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [profileRes, upcomingRes, pastRes, packagesRes, notificationsRes] =
        await Promise.all([
  ...
  const fetchCourseSchedules = useCallback(
    async (courseId: number) => {
      try {
        const response = await axios.get(
          `${apiUrl}/api/public/course-schedules/course/${courseId}`
        );
  ...
  const handleMarkNotificationAsRead = async (id: number) => {
    ...
    try {
      await axios.post(
        `${apiUrl}/api/users/notifications/${id}/mark-read`,
        {},
        { headers }
      );
  ...
  const filteredBookings = useMemo(() => {
    const sourceBookings =
      activeTab === "upcoming" ? allUpcomingBookings : allPastBookings;
  ...
  const handleScheduleSubmit = async () => {
    ...
    try {
      await axios.post(
        `${apiUrl}/api/users/schedule-from-package`,
        requestBody,
        { headers }
      );
            ...
            <div className="bg-white rounded-lg shadow p-4 space-y-2 sticky top-24">
              ...
              <div className="border-t pt-2 mt-2">
                <button
                  ...
                  className="w-full flex items-center px-4 py-3 text-left text-sm font-medium text-red-600 hover:bg-red-50 rounded-md transition-colors"
                >
                  <LogoutIcon /> Logout
                </button>
              </div>
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    ...
                    <button
                      ...
                      className="p-2 bg-gray-200 hover:bg-gray-300 rounded-md text-sm font-medium col-span-full md:col-span-1"
                    >
                      Reset Filters
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  onChange={(e) =>
                    handleCourseSelectForScheduling(e.target.value)
                  }
              ...
              {courseToSchedule &&
                courseSchedules[courseToSchedule]?.length > 0 && (
                  <div>
