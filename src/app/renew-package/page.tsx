...
const RenewalCheckoutForm: React.FC<{ details: PackageRenewalDetails }> = ({
  details,
}) => {
  ...
  return (
    <form id="payment-form" onSubmit={handleSubmit} className="space-y-6">
      ...
      <button
        ...
        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-lg flex justify-center items-center"
      >
        {isLoading ? (
          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
        ) : (
          `Pay $${details.renewalPrice.toFixed(2)} ${details.currency}`
        )}
      </button>
    </form>
  );
...
const RenewPackageContent: React.FC = () => {
  const [renewalDetails, setRenewalDetails] =
    useState<PackageRenewalDetails | null>(null);
    ...
    const prepareRenewal = async () => {
      ...
      try {
        const response = await axios.get(
          `${apiUrl}/api/users/packages/renewal-details/${purchasedPackageId}`,
          { headers }
        );
        ...
        if (axios.isAxiosError(err)) {
          setError(
            err.response?.data || "Could not prepare your package renewal."
          );
        } else {
  ...
  const options: StripeElementsOptions = {
    clientSecret: renewalDetails.clientSecret,
      ...
      <div className="max-w-4xl mx-auto py-12 px-4">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-lg text-gray-600">
            Securely renew your package and continue your training.
          </p>
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="mt-2">
                ...
                <ul className="list-disc list-inside text-sm text-gray-500 pl-4">
                  {renewalDetails.includedCourses.map((course) => (
              ...
              <div className="flex justify-between text-2xl font-bold text-gray-900">
                <span>Total:</span>
                <span>
                  ${renewalDetails.renewalPrice.toFixed(2)}{" "}
                  {renewalDetails.currency}
                </span>
              </div>
            </div>
          </div>
